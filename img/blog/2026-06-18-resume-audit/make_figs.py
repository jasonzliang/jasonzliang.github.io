...
# --- data (verified) -------------------------------------------------------
CLAIMED = 8
# The three glosses are the audit tool's own categories. "Phantom" does not
# mean the work never happened, it means git contradicts the claim: two were
# files the run created in the same commit it claimed to reuse them from, and
# one edge pointed backwards in time.
SEGMENTS = [
    ("Verified", 2, BLUE, "git confirms it"),
    ("Unsupported", 3, GREY, "git cannot confirm it"),
    ("Phantom", 3, RED, "git contradicts it"),
]
...
ax.set_title("Eight claimed influences, checked against git at iteration 6",
             loc="left", pad=30)
ax.text(0, 1.62, "Two confirmed, three refuted, three the check cannot settle "
                 "either way", ha="left", va="center", fontsize=12.5,
