...
agents actually get better, and none of it went into the file. So I tried
deriving the bullets from cited papers instead of from my taste. What came out
is less interesting than how I checked it.
...
bullet leans on, were also checked against their abstracts, and **4 of 4 were
supported verbatim**.
...
is the minimum bar. The gate being mechanical is the point: every identifier it
resolved is in the ledger above, so anyone can rerun it.
...
because they were judged unimportant but because they predate it. So Part A is
distilled from a window-normalized proxy list, not a literal citation-ranked top
ten, which changes what the Part A values are evidence *of*: they reflect what
